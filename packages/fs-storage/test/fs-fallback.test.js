...
describe('fallback to fs for reading', () => {
    ...
    beforeAll(async () => {
        ...
        await writeFile(resolve(storage.keyValueStoresDirectory, 'non-input/some-key.json'), JSON.stringify({ foo: 'bar but from fs' }));
    });
    afterAll(async () => {
    ...
    test('a tracked INPUT record shadows the bare INPUT.json variant in listKeys', async () => {
        ...
        await writeFile(resolve(storage.keyValueStoresDirectory, 'input-collision/INPUT.json'), JSON.stringify({ foo: 'bare' }));
...
describe('run-input bare-file reachability (one variant per store)', () => {
    ...
    const storeNameFor = (file) => `reach-${file.toLowerCase().replace('.', '-')}`;
    beforeAll(async () => {
    ...
    describe.each(BARE_VARIANTS)('a bare $file', ({ file, literalKey, contentType, payload }) => {
        ...
        const unreachableKeys = BARE_VARIANTS.map((variant) => variant.literalKey).filter((key) => !reachableKeys.includes(key));
        test.each(reachableKeys)('is reachable via %s', async (key) => {
            ...
            expect(await store.getValue(key)).toStrictEqual({
                key,
...
describe('run-input bare-file reachability (all variants in one store)', () => {
    ...
    test.each(BARE_VARIANTS)('the literal key $literalKey reads its own file, not a sibling', async ({ literalKey, contentType, payload }) => {
        const store = await storage.createKeyValueStoreBackend({ name: 'all-variants' });
        expect(await store.getValue(literalKey)).toStrictEqual({
            key: literalKey,
            value: Buffer.from(payload),
            contentType,
        });

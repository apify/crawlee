    ...
    private applyClassMethodPatch(patch: ClassMethodPatchDefinition): (original: any) => any {
        ...
        return function wrap(original: (...args: unknown[]) => any) {
            return async function wrapped(this: any, ...args: unknown[]) {
                const spanName =
                    typeof patch.spanName === 'function'
                        ? patch.spanName(this, ...args)
                        : (patch.spanName ?? `${this.constructor.name}.${original.name}`);

                const spanAttributes =
                    typeof patch.spanOptions === 'function'
                        ? patch.spanOptions(this, ...args)
                        : (patch.spanOptions ?? {});
    ...
    private _getLogPatch() {
        return function wrapLog(original: (message: string, ...args: unknown[]) => void) {
            return async function wrappedLog(this: any, level: any, message: string, data?: any, exception?: any) {
                // weak typing to avoid importing apify/log types or redefining them

    ...
    if (isLambdaEnvironment) {
        ...
        try {
            let [totalBytesStr, usedBytesStr] = await Promise.all([
                promises_1.default.readFile(MEMORY_FILE_PATHS.TOTAL[cgroupsVersion], 'utf8'),
                promises_1.default.readFile(MEMORY_FILE_PATHS.USED[cgroupsVersion], 'utf8'),
            ]);
            ...
            if (totalBytesStr === 'max') {
                totalBytes = node_os_1.default.totalmem();
            ...
            log_1.default.deprecated('Your environment is Docker, but your system does not support memory cgroups. '
                ...
                + `Cause: ${err.message}`);
            totalBytes = node_os_1.default.totalmem();
            freeBytes = node_os_1.default.freemem();

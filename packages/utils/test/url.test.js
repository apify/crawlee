...
describe('matchesEnqueueStrategy', () => {
    const match = (strategy, target, origin) => matchesEnqueueStrategy(strategy, new URL(target), new URL(origin));
    ...
    it('treats a trailing-dot FQDN host as equal to the bare host across strategies', () => {
        for (const strategy of ['same-hostname', 'same-domain', 'same-origin']) {

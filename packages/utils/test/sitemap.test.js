...
describe('Sitemap', () => {
    ...
    it('extracts metadata from sitemaps', async () => {
        const items = [];
        for await (const item of parseSitemap([{ type: 'url', url: 'http://not-exists.com/sitemap_child.xml' }], undefined, { httpClient: new FetchHttpClient() })) {
            items.push(item);
        }
    ...
    it('respects nestedSitemapFilter when following sitemap indexes', async () => {
        const items = [];
        for await (const item of parseSitemap([{ type: 'url', url: 'http://not-exists.com/sitemap_parent.xml' }], undefined, {
    ...
    it('keeps quiet if autodetection does not find anything', async () => {
        const logger = { warning: vi.fn(), warningOnce: vi.fn() };
    ...
    it('extracts metadata from pretty-printed XML', async () => {
        const items = [];
        for await (const item of parseSitemap([{ type: 'url', url: 'http://not-exists.com/sitemap_pretty.xml' }], undefined, { httpClient: new FetchHttpClient() })) {
            items.push(item);
        }
    ...
    it('loads sitemaps that reference other sitemaps from string', async () => {
        const sitemap = await Sitemap.fromXmlString([
            ...
            '</sitemap>',
            '</sitemapindex>',
        ].join('\n'), undefined, { httpClient: new FetchHttpClient() });
        expect(new Set(sitemap.urls)).toEqual(new Set([
            'http://not-exists.com/',
            'http://not-exists.com/catalog?item=12&desc=vacation_hawaii',
            'http://not-exists.com/catalog?item=73&desc=vacation_new_zealand',
    ...
    it('logs one aggregated warning per sitemap for dropped URL entries', async () => {
        ...
        await Sitemap.load('http://not-exists.com/cross_host_urls.xml', undefined, {
            logger: { warning, debug: vi.fn(), info: vi.fn(), error: vi.fn() },
        });
        expect(warning).toHaveBeenCalledTimes(1);
        expect(warning).toHaveBeenCalledWith(expect.stringContaining('Skipped 1 URL(s) from sitemap http://not-exists.com/cross_host_urls.xml'));
    });
    it('keeps cross-host URL entries when enqueueStrategy is "all"', async () => {
        ...
        expect(new Set(sitemap.urls)).toEqual(new Set(['http://not-exists.com/local-page', 'http://other.test/cross-page']));
    });

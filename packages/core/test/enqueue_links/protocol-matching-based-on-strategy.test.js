 ...
 */
async function enqueuedUrls(strategy, baseUrl) {
    const enqueued = [];
...
describe('enqueueLinks() - matching and ignoring http/https protocol differences', () => {
    ...
    test('SameHostname should ignore protocol difference', async () => {
        expect(await enqueuedUrls(EnqueueStrategy.SameHostname, 'https://example.com')).toEqual(new Set(['https://example.com/first', 'http://example.com/second']));
    });
    test('SameDomain should ignore protocol difference', async () => {
        expect(await enqueuedUrls(EnqueueStrategy.SameDomain, 'http://example.com')).toEqual(new Set(['https://example.com/first', 'http://example.com/second']));
    });
    test('SameOrigin should respect protocol', async () => {
        expect(await enqueuedUrls(EnqueueStrategy.SameOrigin, 'https://example.com')).toEqual(new Set(['https://example.com/first']));
    });
});

import { CheerioCrawler, MemoryStorageBackend, RequestQueue, serviceLocator, } from '@crawlee/cheerio';
 ...
 */
async function runEnqueueLinks({ selector = 'a', ...options }, requestManager) {
    const enqueued = [];
...
describe("enqueueLinks() - userData shouldn't be changed and outer label must take priority", () => {
    ...
    test('multiple enqueues with different labels', async () => {
        ...
        const byUrl = Object.fromEntries([...first, ...second].map((r) => [r.url, r.userData?.label]));

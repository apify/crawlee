import { CheerioCrawler, EnqueueStrategy, MemoryStorageBackend, RequestQueue, serviceLocator, } from 'crawlee';
 ...
 * resulting URLs.
 */
async function enqueuedUrls(options) {
    const enqueued = [];
...
describe('enqueueLinks() - combining user patterns with enqueue strategies', () => {
    ...
    test('works with include and same domain strategy', async () => {
        const include = ['**/first'];
        expect(await enqueuedUrls({ include, strategy: EnqueueStrategy.SameDomain })).toEqual(new Set(['https://example.com/a/b/first']));
    });
    test('works with include and all domains strategy', async () => {
        const include = ['**/first'];
        expect(await enqueuedUrls({ include, strategy: EnqueueStrategy.All })).toEqual(new Set(['https://example.com/a/b/first', 'https://another.com/a/first']));
    });
    test('works with no user provided patterns but with same domain strategy', async () => {
        expect(await enqueuedUrls({ strategy: EnqueueStrategy.SameDomain })).toEqual(new Set(['https://example.com/a/b/first', 'https://example.com/a/b/third']));
    });
    ...
    test('works with exclude only', async () => {
        ...
        expect(await enqueuedUrls({ exclude, strategy: EnqueueStrategy.All })).toEqual(new Set(['https://example.com/a/b/first', 'http://cool.com/']));
    });
});

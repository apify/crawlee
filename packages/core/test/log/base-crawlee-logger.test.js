...
class TestLogger extends BaseCrawleeLogger {
    logWithLevel(_level, _message, _data) {
...
describe('BaseCrawleeLogger', () => {
    ...
    describe('exception()', () => {
        test('logs at ERROR level with combined message', () => {
            ...
            expect(spy).toHaveBeenCalledWith(LogLevel.ERROR, 'Save failed: disk full', expect.objectContaining({ stack: err.stack }));
        });
        test('merges extra data alongside stack', () => {
            const { logger, spy } = makeLogger();
            const err = new Error('timeout');
            logger.exception(err, 'Request failed', { url: 'https://example.com' });
            expect(spy).toHaveBeenCalledWith(LogLevel.ERROR, 'Request failed: timeout', expect.objectContaining({ url: 'https://example.com', stack: err.stack }));
        });
    });
    describe('softFail()', () => {
    ...
    describe('child()', () => {
        ...
        test('child has independent warningOnce deduplication', () => {
            const { logger } = makeLogger();
            const child = logger.child({ prefix: 'Child' });
            const childSpy = vitest.spyOn(child, 'logWithLevel');

...
/**
 * Minimal cookie jar contract, covering only what Crawlee itself calls. Structurally
 * compatible with `tough-cookie`'s `CookieJar`, which is what backs {@apilink Session} by
 * default, but `@crawlee/types` does not depend on `tough-cookie` for it.
 */
export interface CookieJar {
    ...
    getCookiesSync(url: string): SessionCookie[];

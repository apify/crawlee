...
[What is an Actor](https://sdk.apify.com/docs/guides/what-is-an-actor).
...
There are 3 crawler classes available for use in the Apify SDK. [`BasicCrawler`](/docs/api/basic-crawler), [`CheerioCrawler`](https://sdk.apify.com/docs/api/cheerio-crawler)
and [`PuppeteerCrawler`](https://sdk.apify.com/docs/api/puppeteer-crawler). We'll talk about their differences later. Now, let's talk about what they have in common.
...
All crawlers use instances of the [`Request`](https://sdk.apify.com/docs/api/request) class to determine where they need to go. Each request may hold a lot of information,
...
A representation of the pre-existing list is an instance of the [`RequestList`](https://sdk.apify.com/docs/api/request-list) class. It is a static, immutable list of URLs and
other metadata (see the [`Request`](https://sdk.apify.com/docs/api/request)%20object) that the crawler will visit, one by one, retrying whenever an error occurs, until there
are no more `Requests` to process.

[`RequestQueue`](https://sdk.apify.com/docs/api/request-queue) on the other hand, represents a dynamic queue of `Requests`. One that can be updated at runtime by adding more
...
related metadata) and the `autoscaledPool` property, which is an instance of the [`AutoscaledPool`](https://sdk.apify.com/docs/api/autoscaled-pool) class and we'll talk about
it in detail later.
...
a crawler. We will also use the [`Apify.main()`](https://sdk.apify.com/docs/api/apify#main) function. It's not mandatory, but it makes our life easier. We'll
...
The [`requestQueue.addRequest()`](https://sdk.apify.com/docs/api/request-queue#addRequest) function automatically converts the plain object we passed to it to a
...
[`CheerioCrawler`](https://sdk.apify.com/docs/api/cheerio-crawler) comes in.

#### Overview

[`CheerioCrawler`](https://sdk.apify.com/docs/api/cheerio-crawler) crawls by making plain HTTP requests to the provided URLs. As you remember from the previous section, the
URLs are fed to the crawler using either the [`RequestList`](/docs/api/cheerio-crawler) or the [`RequestQueue`](https://sdk.apify.com/docs/api/cheerio-crawler). The HTTP responses
...
> [`PuppeteerCrawler`](https://sdk.apify.com/docs/api/puppeteer-crawler) to crawl those.
...
#### When to use [`CheerioCrawler`](https://sdk.apify.com/docs/api/cheerio-crawler)
...
scraper, such as the [`PuppeteerCrawler`](https://sdk.apify.com/docs/api/puppeteer-crawler), you'd need significantly more computing power.
...
#### Basic use of [`CheerioCrawler`](https://sdk.apify.com/docs/api/cheerio-crawler)
...
This should be easy, because we already did that [earlier](https://sdk.apify.com/docs/guides/#puttingitalltogether),%20remember?%20Just%20call%20%60requestQueue.addRequest()` for all the new
...
> If you need help with running the code, refer back to the chapters on environment setup: [Setting up locally](https://sdk.apify.com/docs/guides/#settinguplocally) and
> [Setting up on the Apify Platform](https://sdk.apify.com/docs/guides/#settingupontheapifyplatform).
...
[find the full reference here](https://sdk.apify.com/docs/api/utils#enqueueLinks).
...
The `options.pseudoUrls` argument is always an `Array`, but its contents can take on many forms. [See the reference](https://sdk.apify.com/docs/api/utils#enqueueLinks)
...
[reference documentation](https://sdk.apify.com/docs/api/utils#enqueueLinks) to see what else it can do for you. Namely the feature to prepopulate the `Request`
...
![data to scrape](https://sdk.apify.com/img/getting-started/scraping-practice.jpg%20'Overview%20of%20data%20to%20be%20scraped.')
...
It's always good to work step by step. We have this new enqueueing logic in place and since the previous [Sanity check](https://sdk.apify.com/docs/guides/#sanity-check) worked only
...
![actor title](https://sdk.apify.com/img/getting-started/title-01.jpg%20'Finding%20actor%20title%20in%20DevTools.')
...
![actor description selector](https://sdk.apify.com/img/getting-started/description.jpg%20'Finding%20actor%20description%20in%20DevTools.')
...
![actor last run date selector](https://sdk.apify.com/img/getting-started/last-run-date.jpg%20'Finding%20actor%20last%20run%20date%20in%20DevTools.')
...
[`Apify.pushData()`](/docs/api/apify#pushData) is a helper function that saves data to the default [`Dataset`](https://sdk.apify.com/docs/api/dataset). `Dataset` is a
...
than a key in the default [`KeyValueStore`](https://sdk.apify.com/docs/api/key-value-store) that's, by convention, used as input on Apify Platform. Also by convention, the
...
and by calling [`Apify.getInput()`](https://sdk.apify.com/docs/api/apify#getValue) you retrieve the value from the `KeyValueStore`.
...
We wont go into great lengths here to talk about `utils.log` here, because you can read [it all in the documentation](https://sdk.apify.com/docs/api/log), but there's just

...
const ghCommand = (kind, message) => {
    if (github)
        console.log(`::${kind}::${ghEscape(message)}`);
};
...
const sanitizeDts = (content) => content
    .split('\n')
    .filter((line) => !TS_IGNORE_LINE.test(line))
    .join('\n')
    .replace(IGNORE_TAG, '$1$2@internal');
const isImport = (node) => ts.isImportDeclaration(node) || ts.isImportEqualsDeclaration(node);
...
function importBindings(node) {
    ...
    if (bound && ts.isNamespaceImport(bound))
        names.push(bound.name.text);
    if (bound && ts.isNamedImports(bound))
        names.push(...bound.elements.map((element) => element.name.text));
    return names;
}
/**
...
function parseReport(report) {
    ...
    if (open === -1 || close <= open)
        return undefined;
    const source = ts.createSourceFile('report.ts', lines.slice(open + 1, close).join('\n'), ts.ScriptTarget.Latest, true, ts.ScriptKind.TS);
...
function pruneUnusedImports(report) {
    ...
    for (const node of unused) {
        for (let line = lineOf(node.getStart(source)); line <= lineOf(node.getEnd()); line++)
            dropped.add(line);
    }
    ...
    if (unused.length === imports.length && lines[after] === '')
        dropped.add(after);
    ...
    const record = (node) => {
        ...
        if (ts.isIdentifier(node) && !isDeclarationName(node)) {
            ...
            if (positions)
                ...
                occurrences.set(node.text, [node.getStart(source)]);
        }
        node.forEachChild(record);
    };
    source.forEachChild(record);
    const dead = source.statements.filter((statement) => {
        if (isImport(statement) || ts.isExportDeclaration(statement))
            return false;
        ...
        const names = ts.isVariableStatement(statement)
            ...
            : [statement.name];
        if (names.length === 0 || !names.every((name) => name && ts.isIdentifier(name)))
            return false;
        const from = statement.getStart(source);
        const to = statement.getEnd();
        return names.every((name) => forgotten.has(name.text)
...
function annotateForgottenDeclarations(report, forgotten) {
    ...
    for (const statement of source.statements) {
        ...
        const names = ts.isVariableStatement(statement)
            ? statement.declarationList.declarations.map((declaration) => declaration.name)
            : [statement.name];
        if (!names.length || !names.every((name) => name && ts.isIdentifier(name) && forgotten.has(name.text)))
            continue;
        ...
        if (lines[at] !== NOT_EXPORTED_BANNER)
            marked.add(at);
    }
    if (marked.size === 0)
        return report;
...
function getMirror() {
    ...
    for (const pkgJsonPath of packageJsonPaths) {
        ...
        if (existsSync(resolve(dir, 'dist/index.d.ts')))
            paths[manifest(pkgJsonPath).name] = [resolve(dir, 'dist/index.d.ts')];
    }
...
function extract(pkgDir, pkgJsonPath, entry, paths) {
    ...
    if (apiReportChanged && !verify)
        writeFileSync(committedPath, staged);
...
function main() {
    ...
    const expectedReports = new Set(packageJsonPaths
        .map(manifest)
    ...
    try {
        for (const pkgJsonPath of packageJsonPaths) {
            ...
            if (pkg.private || EXCLUDED.has(pkg.name))
                continue;

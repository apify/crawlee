...
StagehandCrawler requires an API key for the AI model provider. The recommended way is to use the `modelApiKey` option:
...
Stagehand options can alternatively be set via environment variables. Programmatic options take precedence, except for LLM API keys where environment variables override `modelApiKey`:

| Environment variable | Option | Notes |
|---------------------|--------|-------|
| `OPENAI_API_KEY` | `modelApiKey` | Overrides `modelApiKey` for OpenAI models |
| `ANTHROPIC_API_KEY` | `modelApiKey` | Overrides `modelApiKey` for Anthropic models |
| `GOOGLE_API_KEY` | `modelApiKey` | Overrides `modelApiKey` for Google models |
| `STAGEHAND_ENV` | `env` | |

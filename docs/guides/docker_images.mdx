...
### Start with the full image, switch to slim when done

While developing, use the full image. It comes with `apify`, `crawlee` and `typescript` preinstalled, so you can iterate without worrying about which dependencies your Dockerfile needs to install. Once the crawler is stable and your `package.json` lists everything it needs, switch the tag to the [`-slim` variant](#slim-variants) for a smaller image and faster builds.

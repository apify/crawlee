...
With that warning aside, this is how we pass those options. Two things to know: an injected `ConcurrencySystem` is ours to `start()` and `stop()`, and the `minConcurrency`/`maxConcurrency`/`maxRequestsPerMinute` shortcuts cannot be combined with one — they configure the default system it replaces, so set those limits on the instance instead:
...
Two signals cannot share a name, and a duplicate throws at construction time. Taking over one of the built-in names (`memInfo`, `eventLoopInfo`, `cpuInfo`, `clientInfo`) therefore means switching that built-in off first.

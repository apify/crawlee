...
When a request is handled, its `userData` is validated against the schema registered for its label and replaced with the parsed (and coerced) value before the handler runs. The same schemas also validate `userData` **when requests are added** — via `crawler.addRequests()`, `crawler.run()`, the `addRequests`/`enqueueLinks` context helpers, or a direct `requestQueue.addRequest()` — so a bad shape fails fast at the point of insertion rather than deep inside a handler. A request whose `userData` does not match its label's schema throws a <ApiLink to="core/class/RequestValidationError">`RequestValidationError`</ApiLink> (a non-retryable error, since re-running it would fail identically). Labels without a registered schema are left untouched.

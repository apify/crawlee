...
When you structure a crawler with a <ApiLink to="basic-crawler/class/Router">`Router`</ApiLink>, different labels usually carry a different shape of `request.userData` — a `PRODUCT` request might hold `{ sku, price }` while a `CATEGORY` request holds `{ categoryId }`. By default `userData` is loosely typed (`Record<string, unknown>`), so a typo in a label or a wrong `userData` property is only caught at runtime.

You can instead declare a **route map** — an `interface` (or `type`) that maps each label to the shape of `userData` expected for it — and pass it as the second type argument to a `createXRouter` factory (or <ApiLink to="basic-crawler/class/Router#create">`Router.create`</ApiLink>). Handlers then get `request.userData` typed per label, and unknown labels become a compile-time error:
...
When a request is handled, its `userData` is validated against the schema registered for its label and replaced with the parsed (and coerced) value before the handler runs. The same schemas also validate `userData` **when requests are added** through the crawler — via `crawler.addRequests()`, `crawler.run()`, and the `addRequests`/`enqueueLinks` context helpers — so a bad shape fails fast at the point of insertion rather than deep inside a handler, and the queue stores the parsed value rather than the raw input. A request whose `userData` does not match its label's schema throws a <ApiLink to="core/class/RequestValidationError">`RequestValidationError`</ApiLink> (a non-retryable error, since re-running it would fail identically). Labels without a registered schema are left untouched.

Crawlee stores the request's `label` inside `userData`, but validating it is **opt-in**: schemas that don't describe `label` (including `z.strictObject()` ones) are unaffected by it, and the label is preserved even when a schema strips undeclared keys. Declare it if you want it checked — useful as a discriminator:

```ts
const router = createCheerioRouter({
    // the label is validated only because the schema asks for it
    PRODUCT: z.object({ label: z.literal('PRODUCT'), sku: z.string() }),
});
```

To validate requests handled by the **default route** — those whose label has no route of its own — register a schema under the exported `defaultRoute` key. It is used only for validation, so it does not add a label to the inferred route map:
...
router.addDefaultHandler(async ({ request }) => {
    request.userData.page; // validated (and coerced) by the defaultRoute schema
});
```
...
The schema map and the type-only route map are two alternative styles — a schema map already infers the `userData` types, so you don't also pass a `<Context, Routes>` type argument. Pick one: `createCheerioRouter<Context, Routes>()` for compile-time typing only, or `createCheerioRouter({ LABEL: schema })` for typing plus runtime validation.

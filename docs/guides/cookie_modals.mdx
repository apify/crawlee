...
<ApiLink to="basic-crawler/class/Session#setCookie">`session.setCookie`</ApiLink> writes into the session's <ApiLink to="basic-crawler/class/Session#cookieJar">`cookieJar`</ApiLink>. The crawler copies that jar into the browser just before navigating, so a call inside a <ApiLink to="playwright-crawler/interface/PlaywrightCrawlerOptions#preNavigationHooks">`preNavigationHook`</ApiLink> arrives in time.

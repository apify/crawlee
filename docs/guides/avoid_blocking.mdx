...
If you need finer control, both `preNavigationHooks` and `postNavigationHooks` may return a partial object whose properties are merged into the crawling context. This lets a hook produce a fresh `response` (e.g. by reloading the page after solving a challenge) that's observed by status-code validation, subsequent hooks and the request handler.

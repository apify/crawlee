...
Changes how crawlee measures its CPU and Memory usage and limits. If unset, crawlee will determine if it is containerised using common features of containerized environments.
 - A file at `/.dockerenv`.
...
Setting this environment variable overwrites the behaviour of `isContainerized`. Most values will make `isContainerized` return `true`.
Setting this environment variable to `false` will make `isContainerized` return `false`.

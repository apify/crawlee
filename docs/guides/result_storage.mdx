...
By default, Crawlee storage is managed by the <ApiLink to="fs-storage/class/FileSystemStorageClient">`FileSystemStorageClient`</ApiLink> class, which stores all information as local files in the respective storage type folders.

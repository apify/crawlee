...
Two things follow for anyone implementing the interface. The method is required, so reporting a signal is never a question of whether a manager supports it: one that does not pace — a plain queue — returns `false`, and the crawler warns that the signal was dropped. A manager that **wraps** another one forwards it, as <ApiLink to="core/class/RequestManagerTandem">`RequestManagerTandem`</ApiLink> does, or a pacer nested inside a composition goes deaf; forwarding needs no knowledge of the payload, which is why this is one method taking a value rather than a method per kind of signal.

...
A third `reason`, `minIntervalEverywhere`, is a floor under the pace of **every** domain the manager dispatches to — declared by whoever owns the crawl rather than by a source, which is why it is the one variant carrying no `url`. It is how the crawler offers a manager its <ApiLink to="basic-crawler/interface/BasicCrawlerOptions#sameDomainDelaySecs">`sameDomainDelaySecs`</ApiLink>: whatever paces takes the floor, and only when nothing does the crawler add a pacer of its own. A manager pacing some of its domains but not all of them throws instead of taking a floor it would under-apply.

Two things follow for anyone implementing the interface. The method is required, so reporting a signal is never a question of whether a manager supports it: one that does not pace — a plain queue — returns `false`, and the crawler warns that the signal was dropped. A manager that **wraps** another one forwards it, as <ApiLink to="core/class/RequestManagerTandem">`RequestManagerTandem`</ApiLink> does, or a pacer nested inside a composition goes deaf; forwarding needs no knowledge of the payload, which is why this is one method taking a value rather than a method per kind of signal — and why what a signal applies to travels inside that value.

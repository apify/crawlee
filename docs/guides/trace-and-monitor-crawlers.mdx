...
```bash
npx tsx --experimental-loader=@opentelemetry/instrumentation/hook.mjs --import ./src/setup.ts ./src/main.ts
```

The `--import` flag ensures the OpenTelemetry setup runs before any other code, which is required for the automatic
instrumentation to work properly. The `--experimental-loader` flag installs the OpenTelemetry module hook, which the
instrumentation needs in order to patch the Crawlee classes as they are imported.

:::caution The module hook is required

Crawlee is published as ECMAScript modules, so the automatic instrumentation can only patch the crawler classes when
Node.js is started with the OpenTelemetry loader hook shown above. Without it the crawler runs normally but no spans
are produced. The same applies if the Crawlee packages are inlined into a bundle - there is then no module load left to
intercept, so exclude them from the bundle.

`wrapWithSpan` is unaffected by both, as it wraps your own functions directly rather than relying on patching.

:::

:::caution Linked installs also need `--preserve-symlinks`

Patching additionally requires the package to resolve to a path inside a `node_modules` directory. That holds for a
normal install (npm, yarn and pnpm alike), but not for a package symlinked out to a working copy - `npm link`, a
`file:` dependency, or a workspace in a monorepo. Node resolves those to their real location, which has no
`node_modules` segment, so the hook cannot match the file back to the package name. Add
`--preserve-symlinks --preserve-symlinks-main` in that case, so that the symlinked path is kept.

The examples on this page live in the Crawlee repository, where the packages are workspace symlinks - so this is how
you run the one above from the repository root:

```bash
cd docs && npx tsx --preserve-symlinks --preserve-symlinks-main \
    --experimental-loader=@opentelemetry/instrumentation/hook.mjs \
    --import ./guides/trace_and_monitor_setup.ts ./guides/trace_and_monitor_basic.ts
```

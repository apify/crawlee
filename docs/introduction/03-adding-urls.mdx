...
The default behavior of `enqueueLinks` is to stay on the same domain. This **includes different subdomains**. To not include different subdomains in your crawl, use the `strategy` argument.
...
If you provide one of those options, the default `same-domain` strategy will **not** be applied unless explicitly set in the options.

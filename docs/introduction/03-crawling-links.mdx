...
In the previous lesson you built a very simple crawler that downloads the HTML of a single page, reads its title and prints
...
In this lesson we'll use the example from the previous section and improve on it by letting it truly crawl the page, finding new links as it goes, enqueuing them into the `RequestQueue` and then scraping them.
...
In the next lesson you will start your project of scraping a business website and learn some more Crawlee tricks in the process.

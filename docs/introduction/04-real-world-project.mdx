...
For the purposes of this tutorial, let's assume that the website cannot be scraped with `CheerioCrawler`. It actually can, but we would have to dive a bit deeper than this introductory guide allows. We will make things easier for us, scrape it with `PlaywrightCrawler`, and learn about headless browsers in the process.

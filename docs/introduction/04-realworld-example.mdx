...
Fortunately, you don't have to travel to a galaxy far, far away to find a good candidate for learning how to scrape structured data. We could pick any modern e-commerce website, but we've chosen the [Apify Store](https://apify.com/store). It is a library of public scrapers and automations that anyone can grab and use for free.

:::info

While this definitely is a bit of a shameless self-promotion, the real reason why we chose the Apify Store is quite simple, actually. It's our own website, so we know when it will change and when to update our tutorials. Unlike with third-party websites where it always breaks in the most annoying moment. It also requires JavaScript rendering, which will allow us to show more features of Crawlee.
...
A good first step is to figure out what data you want to scrape and where to find it. For the time being, let's just agree that we want to scrape all actors from [Apify Store](https://apify.com/store) in all categories (which can be found on the left side of the page) and for each actor we want to get its:

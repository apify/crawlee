...
The code looks complicated, but it only reads that we're looking for an `<ul class="ActorHeader-userMedallion ...">` element and within that element we're looking for the `<li>` element that includes the text **Runs**. We're only interested in the number of runs. So we extract the number from the last (second) `<li>` element. But it contains a string with "Runs " word as well, so we remove the "Runs " by replacing them with empty strings, .

...
A subclass that overrode `defaultState()` and `deserializeState()` only to track extra fields should declare them via the new `defaultState` *option* instead — see the [Custom statistics fields](../guides/custom-statistics) guide. The subclass keeps working; it is just no longer necessary.

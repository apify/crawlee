...
## `preNavigationHooks` no longer accept `gotOptions` object

The `preNavigationHooks` no longer accept the `gotOptions` object as a second parameter. Modify the `crawlingContext` fields (e.g. `.request`) directly instead.

...
const crawler = new FileDownload({
    async requestHandler({ response, request, log, getKeyValueStore }) {
        ...
        await kvs.setValue(url.pathname.replace(/\//g, '_'), pipeline(response.body, createProgressTracker({ url, log, totalBytes: Number(response.headers.get('content-length')) }), (error) => {
            if (error)

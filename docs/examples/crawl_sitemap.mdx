...
Sitemap crawler is a tool that crawl pages of a website, much like search engine crawlers do, in order to gain valuable SEO information. This example builds a sitemap crawler which downloads and crawls the URLs from a sitemap, by using the <ApiLink to="utils/class/Sitemap">`Sitemap`</ApiLink> utility class provided by the <ApiLink to="utils">`@crawlee/utils`</ApiLink> module.

...
const createLaunchContext = () => new LaunchContext({ id: 'id', browserPlugin: {}, launchOptions: {} });
describe('LaunchContext', () => {
    ...
    test('extend() rejects every declared field as reserved', () => {
        ...
        for (const key of reservedKeys) {
            expect(() => context.extend({ [key]: 'anything' })).toThrow(`Cannot extend LaunchContext with key: ${key}, because it's reserved.`);
        }
    });
    test('declared fields are set directly instead', () => {
        const context = createLaunchContext();
        const fingerprint = { fingerprint: {}, headers: {} };

...
describe('parseArgument', () => {
    ...
    test('message names the offending field and the value it received', () => {
        const schema = z
            ...
            .strict();
        expect(() => parseArgument({ countryCode: 'CZE' }, schema)).toThrow('Invalid string: must match pattern /^[A-Z]{2}$/ at `countryCode`, got `CZE`');
    });
    test('message folds the received type and value into one clause', () => {
        expect(() => parseArgument({ retries: 'three' }, z.strictObject({ retries: schemas.anyNumber }))).toThrow('Invalid input: expected number, received the string `three` at `retries`');
        // `NaN` is named as itself, not as the self-contradictory "received number".
        expect(() => parseArgument(Number.NaN, schemas.anyNumber)).toThrow('Invalid input: expected number, received NaN');
        // An empty string would render as bare backticks — made visible instead.
        expect(() => parseArgument('', schemas.anyNumber)).toThrow('Invalid input: expected number, received an empty string');
        // Long strings are elided rather than dumped whole into the message.
        expect(() => parseArgument('a'.repeat(250), schemas.anyNumber)).toThrow(`Invalid input: expected number, received the string \`${'a'.repeat(200)}… (50 more characters)\``);
    });
    test('arrayOf names the element type at the top level and per element', () => {
        ...
        expect(() => parseArgument({ codes: 500 }, schema)).toThrow('Invalid input: expected an array of numbers, received the number `500` at `codes`');
        // Element failures keep zod's per-index messages.
        expect(() => parseArgument({ codes: [500, 'x'] }, schema)).toThrow('Invalid input: expected number, received the string `x` at `codes[1]`');
    ...
    test('label names the validated interface on every line', () => {
        ...
        expect(() => parseArgument({ retries: 'three', name: 7 }, schema, 'ExampleOptions')).toThrow('Invalid input: expected number, received the string `three` at `retries` in `ExampleOptions`\n' +
    ...
    test('returns typed parsed output for object schemas', () => {
        ...
        const parsed = parseArgument(options, schemas.datasetListItemsOptions);
...
describe('ow parity in crawler options', () => {
    ...
    test('BasicCrawler throws ArgumentValidationError for an unknown key', () => {
        const create = () => new BasicCrawler({ unknownOption: true });
    ...
    test('ProxyConfiguration throws ArgumentValidationError for an unknown key', () => {
        const create = () => new ProxyConfiguration({ unknownOption: true });
        expect(create).toThrow(ArgumentValidationError);
        expect(create).toThrow('Unrecognized key: "unknownOption"');
    });
});

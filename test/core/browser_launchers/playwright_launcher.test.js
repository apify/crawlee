...
import { BrowserLauncher, Configuration, launchPlaywright, PlaywrightLauncher, serviceLocator, } from '@crawlee/playwright';
...
beforeAll(async () => {
    ...
    return portastic.find({ min: 50000, max: 50099 }).then(async (ports) => {
...
afterAll(async () => {
    server.close();
    if (proxyServer)
        await util.promisify(proxyServer.close).bind(proxyServer)();
}, 5000);
describe('launchPlaywright()', () => {
    ...
    test('supports useChrome option', async () => {
        const spy = vitest.spyOn(BrowserLauncher.prototype, 'getTypicalChromeExecutablePath');
        let browser;
        const opts = {
            useChrome: true,

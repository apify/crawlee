import { ArgumentValidationError, deserializeArray, EventType, KeyValueStore, MemoryStorageBackend, ProxyConfiguration, Request, RequestList, REQUESTS_PERSISTENCE_KEY, serviceLocator, STATE_PERSISTENCE_KEY, } from '@crawlee/core';
...
describe('RequestList', () => {
    ...
    test('should correctly load list from hosted files in correct order', async () => {
        const spy = vitest.spyOn(RequestList.prototype, 'downloadListOfUrls');
    ...
    describe('Apify.RequestList.open()', () => {
        ...
        const keysPassedTo = (spy) => spy.mock.calls.map(([key]) => key);

import { launchPlaywright, launchPuppeteer, MemoryStorageBackend, playwrightClickElements, playwrightUtils, puppeteerClickElements, puppeteerUtils, RequestQueue, serviceLocator, } from 'crawlee';
import { runExampleComServer } from '../../shared/_helper.js';
function isPuppeteerBrowser(browser) {
    return browser.targets !== undefined;
}
function isPlaywrightBrowser(browser) {
    return browser.browserType !== undefined;
}
                ...
                const callCounts = await new Promise((resolve) => {
                    if (isPuppeteerBrowser(browser)) {
                        ...
                        browser.on('targetcreated', (target) => {
                            counts.create++;
                            if (clickElements.isTargetRelevant(page, target))

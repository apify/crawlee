...
// Regression test for the `clickOptions` input validator. It does not need a real browser:
// the validation runs synchronously at the very start of `enqueueLinksByClickingElements`,
// before the page is ever touched, so mock `page`/`requestQueue` objects that only satisfy
// the input-shape checks are enough. The declared `ClickOptions` type has all-optional,
// independent fields, but the validator used `ow.optional.object.hasKeys('clickCount', 'delay')`,
// which requires BOTH keys to be present and therefore rejected any single option (e.g. `{ delay }`
// or the modern `{ count }`) with an `ArgumentError` before a single element was clicked.
describe.each([
    { caseName: 'Puppeteer', utils: puppeteerUtils },
    { caseName: 'Playwright', utils: playwrightUtils },
])('$caseName: enqueueLinksByClickingElements() clickOptions validation', ({ utils }) => {
    const page = { goto() {}, evaluate() {} };
    const requestQueue = { fetchNextRequest() {}, addRequest() {} };

    async function validate(clickOptions: Record<string, unknown>) {
        try {
            await utils.enqueueLinksByClickingElements({
                page: page as any,
                requestQueue: requestQueue as any,
                selector: 'a',
                clickOptions: clickOptions as any,
            });
        } catch (e) {
    ...
    test.each([
        ...
        { button: 'right' },
        { clickCount: 2 },
    ])('accepts %o without a keys validation error', async (clickOptions) => {
        // The call still rejects afterwards because `page`/`requestQueue` are mocks, but it must
        // no longer fail on the `clickOptions` validator. Before the fix these all threw
        // "Expected ... `clickOptions` to have keys ...".
        expect(await validate(clickOptions)).not.toMatch(/clickOptions.*to have keys/);
    });
});

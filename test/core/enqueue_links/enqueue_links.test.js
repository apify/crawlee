import { CheerioCrawler, EnqueueStrategy, MemoryStorageBackend, Request, RequestQueue, serviceLocator, } from '@crawlee/cheerio';
...
async function createRequestQueueMock(seedUrl = 'https://example.com') {
    ...
    requestQueue.addRequests = async (requests, options) => {
        ...
        for await (const request of requests) {
            items.push(typeof request === 'string' ? { url: request } : request);
        }
...
async function runCheerioEnqueueLinks(enqueueOptions, { requestManager, startUrl = 'https://example.com' }) {
...
async function runPlaywrightEnqueueLinks(enqueueOptions, { requestManager, startUrl = 'https://example.com' }) {
...
describe('enqueueLinks()', () => {
    ...
    describe('using Playwright', () => {
        test('works with item limit', async () => {
            ...
            await runPlaywrightEnqueueLinks({ limit: 3, selector: '.click', strategy: EnqueueStrategy.All }, { requestManager: requestQueue });
        ...
        test('works with include (globs)', async () => {
            ...
            await runPlaywrightEnqueueLinks({ selector: '.click', strategy: EnqueueStrategy.All, label: 'COOL', include }, { requestManager: requestQueue });
            expect(enqueued).toHaveLength(3);
            expect(enqueued.map((r) => r.url)).toEqual([
                'https://example.com/a/b/first',
                'https://example.com/a/b/third',
        ...
        test('correctly resolves relative URLs with the strategy of same-domain', async () => {
            const { enqueued, requestQueue } = await createRequestQueueMock();
            await runPlaywrightEnqueueLinks({ baseUrl: 'http://www.absolute.com/removethis/', strategy: EnqueueStrategy.SameDomain }, { requestManager: requestQueue });
            expect(enqueued.map((r) => r.url)).toEqual([
    ...
    describe('using Cheerio', () => {
        ...
        test('does not throw with empty include patterns', async () => {
            ...
            await expect(runCheerioEnqueueLinks({ selector: '.click', include, strategy: EnqueueStrategy.All }, { requestManager: requestQueue })).resolves.not.toThrow();
            expect(enqueued).toHaveLength(3);
        });
        ...
        test('works with include (mixed globs and regexps)', async () => {
            const { enqueued, requestQueue } = await createRequestQueueMock();
            const include = ['https://example.com/**/*', /^(http|https):\/\/cool\.com\//];
            await runCheerioEnqueueLinks({ selector: '.click', strategy: EnqueueStrategy.All, include }, { requestManager: requestQueue });
            expect(enqueued).toHaveLength(3);
            expect(enqueued[0].url).toBe('https://example.com/a/b/first');
            expect(enqueued[1].url).toBe('https://example.com/a/b/third');
            expect(enqueued[2].url).toBe('http://cool.com/');
        ...
        test('throws when include is an empty array', async () => {
            ...
            expect(caughtError.message).toMatch(/at `include`/);
        });
        test('works with no include/exclude filters (enqueues all matching strategy)', async () => {
            const { enqueued, requestQueue } = await createRequestQueueMock();
            await runCheerioEnqueueLinks({ selector: '.click', strategy: EnqueueStrategy.All }, { requestManager: requestQueue });
            expect(enqueued.map((r) => r.url)).toEqual([
                'https://example.com/a/b/first',
                'https://example.com/a/b/third',
                'https://another.com/a/fifth',
        ...
        test('works with exclude glob', async () => {
            ...
            const include = ['https://example.com/**/*', '?(http|https)://cool.com/'];
            const exclude = ['**/first'];
            await runCheerioEnqueueLinks({ selector: '.click', strategy: EnqueueStrategy.All, include, exclude }, { requestManager: requestQueue });
        ...
        test('works with skipNavigation', async () => {
            const { enqueued, requestQueue } = await createRequestQueueMock();
            await runCheerioEnqueueLinks({ selector: '.click', skipNavigation: true }, { requestManager: requestQueue });
            expect(enqueued).toHaveLength(2);
            for (const request of enqueued) {
        ...
        test('correctly resolves relative URLs with the default strategy of same-hostname', async () => {
            const { enqueued, requestQueue } = await createRequestQueueMock();
            await runCheerioEnqueueLinks({ baseUrl: 'http://www.absolute.com/removethis/' }, { requestManager: requestQueue });
            expect(enqueued.map((r) => r.url)).toEqual([
                'http://www.absolute.com/x/absolutepath',
                'http://www.absolute.com/removethis/y/relativepath',
            ]);
        ...
        test('correctly resolves relative URLs with the strategy of same-domain', async () => {
            const { enqueued, requestQueue } = await createRequestQueueMock();
            await runCheerioEnqueueLinks({ baseUrl: 'http://www.absolute.com/removethis/', strategy: EnqueueStrategy.SameDomain }, { requestManager: requestQueue });
            expect(enqueued.map((r) => r.url)).toEqual([
                'http://www.absolute.com/x/absolutepath',
                'http://www.absolute.com/removethis/y/relativepath',
                'http://example.absolute.com/hello',
        ...
        test('correctly resolves relative URLs with the strategy of all', async () => {
            const { enqueued, requestQueue } = await createRequestQueueMock();
            await runCheerioEnqueueLinks({ baseUrl: 'http://www.absolute.com/removethis/', strategy: EnqueueStrategy.All }, { requestManager: requestQueue });
            expect(enqueued.map((r) => r.url)).toEqual([
                'https://example.com/a/b/first',
            ...
            requestQueue.addRequests = async (requests, options) => {
                ...
                for await (const request of requests) {
                    ...
                    if (source.url !== 'https://example.com')
                        enqueued.push({ request: source, options });
                }
                return originalAddRequests(items, options);
            };
        ...
        describe('label precedence', () => {
            test('global label option is applied if no other label is provided', async () => {
                const { enqueued, requestQueue } = await createRequestQueueMock();
                await runCheerioEnqueueLinks({ selector: '.click', label: 'global-label', include: ['https://example.com/**/*'] }, { requestManager: requestQueue });
                expect(enqueued).toHaveLength(2);
                ...
                requestQueue.addRequestsBatched = async (requests, options) => {
                    ...
                    for await (const request of requests) {
                        if (!(request instanceof Request)) {
                            throw new Error(`Expected Request instance but got plain object: ${JSON.stringify(request)}`);
                        }
                        items.push(request);
            ...
            test('transformRequestFunction supports "skip" and "unchanged" string returns', async () => {
                ...
                expect(enqueued[0].userData).toEqual({ label: 'global-label' });
                const skippedCalls = onSkippedRequest.mock.calls.map((call) => call[0]);
    ...
    describe('respectRobotsTxtFile option', () => {
        const robotsTxtFile = RobotsTxtFile.from('http://example.com/robots.txt', `User-agent: *
             Disallow: /
             Allow: /yes
        ...
        async function runWithRobotsTxt(respectRobotsTxtFile) {
            const { enqueued, requestQueue } = await createRequestQueueMock();
            const crawler = new (class extends CheerioCrawler {
                async getRobotsTxtFileForUrl(_url) {

...
describe('enqueueLinks()', () => {
    ...
    describe.each([[launchPuppeteer], [launchPlaywright]] as const)('using %s', (method) => {
        ...
        test('works with pseudoUrls', async () => {
            const { enqueued, requestQueue } = createRequestQueueMock();
            const pseudoUrls = ['https://example.com/[(\\w|-|/)*]', { purl: '[http|https]://cool.com/', method: 'POST' as const, userData: { label: 'COOL' } }];
    ...
    describe('using Cheerio', () => {
        ...
        test('works with globs', async () => {
            const { enqueued, requestQueue } = createRequestQueueMock();
            const globs = ['https://example.com/**/*', { glob: '?(http|https)://cool.com/', method: 'POST' as const, userData: { label: 'COOL' } }];

...
import { BasicCrawler, Configuration, CriticalError, Dataset, defaultRoute, ErrorTracker, EventType, KeyValueStore, MissingRouteError, NonRetryableError, ProxyConfiguration, Request, RequestList, RequestQueue, RequestValidationError, Router, serviceLocator, SessionPool, Statistics, ThrottlingRequestManager, } from '@crawlee/basic';
...
describe('BasicCrawler', () => {
    ...
    beforeEach(async () => {
        ...
        requestQueueBackend = memoryRequestQueue.backend;
    });
    afterAll(async () => {
    ...
    test('run() broadcasts start and terminal STATUS_MESSAGE events', async () => {
        const events = serviceLocator.getEventManager();
        const messages = [];
        const listener = (data) => messages.push(data.message);
        events.on(EventType.STATUS_MESSAGE, listener);
        try {
            const crawler = new BasicCrawler({
                requestHandler: () => { },
    ...
    test('should run in parallel thru all the requests', async () => {
        ...
        expect(basicCrawler.concurrencySystem.minConcurrency).toBe(25);
    ...
    test('folds a supplied concurrencySystem into its pool and never tears the system down', async () => {
        ...
        expect(basicCrawler.concurrencySystem.minConcurrency).toBe(7);
    ...
    test.each(['minConcurrency', 'maxConcurrency', 'maxRequestsPerMinute'])('throws when %s is combined with a supplied concurrencySystem', (shortcut) => {
        expect(() => new BasicCrawler({
    ...
    test('two crawlers sharing a ConcurrencySystem cap their combined concurrency', async () => {
        ...
        let combinedPeak = 0;
        const makeCrawler = async (offset) => {
    ...
    test('builds a fresh owned ConcurrencySystem for every run', async () => {
        ...
        const firstSystem = crawler.concurrencySystem;
    ...
    test('stops the owned ConcurrencySystem when startup fails after it was started', async () => {
        ...
        expect(crawler.concurrencySystem.isRunning).toBe(false);
    ...
    test('addRequests should respect maxCrawlDepth', async () => {
        const processedUrls = [];
        const requestHandler = async ({ request, addRequests }) => {
    ...
    describe('addRequests() filters', () => {
        let onSkippedRequestMock;
        let addRequestsBatchedMock;
        let drainedRequests;
        let options;
        let requestQueue;
        ...
        const atDepth = (urls, crawlDepth) => urls.map((url) => ({ url, crawlDepth }));
        beforeEach(() => {
            drainedRequests = [];
            ...
            addRequestsBatchedMock = vi.fn().mockImplementation(async (requests) => {
            ...
            requestQueue = {
                addRequestsBatched: addRequestsBatchedMock,
            };
        ...
        it('should respect user provided transformRequestFunction', async () => {
            const urls = atDepth(['https://example.com/1/', 'https://example.com/2/'], 3);
            const transformRequestFunction = vi.fn((req) => req);
        ...
        it.each([null, undefined, false])('should skip requests when transformRequestFunction returns %s', async (returnValue) => {
            const urls = atDepth(['https://example.com/1/', 'https://example.com/2/'], 3);
    ...
    it('addCrawlDepthRequestGenerator() should generate requests with maxCrawlDepth', async () => {
        class TestCrawler extends BasicCrawler {
            exposedAddCrawlDepthRequestGenerator(...enqueueLinksOptions) {
    ...
    test('concurrency shortcuts configure the default system; an injected system is used as-is', async () => {
        ...
        const requestHandler = async () => { };
        const collect = (crawler) => ({
            minConcurrency: crawler.concurrencySystem.minConcurrency,
            maxConcurrency: crawler.concurrencySystem.maxConcurrency,
            // eslint-disable-next-line dot-notation -- private member on the governor
            maxTasksPerMinute: crawler.concurrencySystem['maxTasksPerMinute'],
        });
    ...
    test('auto-saved state object', async () => {
        ...
        const processed = [];
        const requestList = await RequestList.open(null, sources);
        const requestHandler = async ({ request, useState }) => {
            await sleep(10);
    ...
    test('print a warning on sharing state between two crawlers', async () => {
        function createCrawler() {
            return new BasicCrawler({
                requestHandler: async ({ request, useState }) => {
                    const state = await useState({ urls: [] });
        ...
        const state1 = await crawler1.useState();
        const state2 = await crawler2.useState();
    ...
    test('shared-state warning is emitted only once regardless of crawler count', async () => {
        ...
        const sharedStateWarnings = warningSpy.mock.calls.filter(([msg]) => typeof msg === 'string' && msg.includes('Multiple crawler instances are calling useState()'));
        expect(sharedStateWarnings).toHaveLength(1);
    });
    test('crawlers with explicit id have isolated state', async () => {
        function createCrawler(id) {
            return new BasicCrawler({
                id,
                requestHandler: async ({ request, useState }) => {
                    const state = await useState({ urls: [] });
    ...
    test.each([EventType.MIGRATING, EventType.ABORTING])('should pause on %s event and persist RequestList state', async (event) => {
        ...
        setValueSpy.mockImplementationOnce(persistResolve);
    ...
    test('should correctly combine RequestList and RequestQueue', async () => {
        ...
        vitest
            ...
            .mockResolvedValueOnce({ requestId: 'id-0' })
            .mockResolvedValueOnce({ requestId: 'id-1' })
            .mockResolvedValueOnce({ requestId: 'id-2' });
        ...
        const reclaimReq = vitest.spyOn(requestQueue, 'reclaimRequest').mockReturnValue(Promise.resolve());
    ...
    test('should be possible to override isFinishedFunction and isTaskReadyFunction via taskLoopOptions', async () => {
        ...
        vitest.spyOn(requestQueue, 'getHandledCount').mockReturnValue(Promise.resolve());
        let handledCount = 0;
    ...
    test('exposes the concurrency system it is running against', async () => {
        ...
        expect(observed.maxConcurrency).toBe(17);
    });
    ...
    test('a route can override requestHandlerTimeoutSecs, other routes keep the default', async () => {
        ...
        router.addHandler('LIST', async ({ request }) => {
            await sleep(300);
            processed.push(request);
    ...
    test('warns when the internal timeout is shorter than the phase timeouts', async () => {
        ...
        await crawler.run();
        const warned = warnSpy.mock.calls.some((call) => String(call[0]).includes('shorter than the navigation and request handler'));
    ...
    describe('Uses SessionPool', () => {
        ...
        it('should share sessions across crawlers using the same SessionPool', async () => {
            ...
            const crawler1Sessions = new Set();
            const crawler2Sessions = new Set();
    ...
    describe('statistics injection', () => {
        ...
        it('does not reset a supplied instance between runs, but does reset a default', async () => {
            ...
            const ownedResetSpy = vitest.spyOn(owningCrawler.statistics, 'reset');
    ...
    describe('sendRequest', () => {
        ...
        test('forwards ignoreTlsErrors to the http client', async () => {
            ...
            const crawler = new BasicCrawler({
                ...
                httpClient: Object.assign(Object.create(BaseHttpClient.prototype), {
                    async sendRequest(_request, options) {
    ...
    describe('Request enqueuing limits with maxRequestsPerCrawl', () => {
        ...
        test('should respect robots.txt when limiting requests', async () => {
            ...
            vitest.spyOn(crawler, 'isAllowedBasedOnRobotsTxtFile').mockImplementation(async (url) => {
        ...
        test.each([
            ...
            const crawler = new (class MockedRobotsTxtCrawler extends BasicCrawler {
                async getRobotsTxtFileForUrl(_) {
        ...
        describe('robots.txt crawl-delay', () => {
            const crawlerWithCrawlDelay = (options) => new (class MockedRobotsTxtCrawler extends BasicCrawler {
                async getRobotsTxtFileForUrl(_) {
                    return RobotsTxtFile.from('http://example.com/robots.txt', 'User-agent: *\nCrawl-delay: 5\n');
                }
            })({ respectRobotsTxtFile: true, requestHandler: async () => { }, ...options });
            test('is applied when a ThrottlingRequestManager covers the domain', async () => {
                ...
                const state = requestManager.domainStates.get('example.com');
        ...
        describe('sameDomainDelaySecs', () => {
            const crawlerVisiting = async (urls, options = {}) => {
                const visits = [];
                const crawler = new BasicCrawler({
            ...
            test('spaces out requests to one site, subdomains included', async () => {
                const { visits } = await crawlerVisiting(['http://a.example.com/1', 'http://b.example.com/2', 'http://other.com/1'], { sameDomainDelaySecs: 1 });
        ...
        test('enqueueLinks forwards respectRobotsTxtFile.userAgent to the robots.txt check', async () => {
            ...
            const crawler = new (class MockedRobotsTxtCrawler extends BasicCrawler {
                async getRobotsTxtFileForUrl(_) {
                    return { isAllowed: isAllowedSpy, getCrawlDelay: () => undefined };
                }
            })({
                requestQueue,
                maxConcurrency: 1,
        ...
        test('enqueueLinks limit log message should only be logged once', async () => {
            ...
            const enqueueLimitMessages = infoSpy.mock.calls.filter((call) => typeof call[0] === 'string' && call[0].includes('enqueueLinks limit'));
        ...
        test('maxCrawlDepth limit log message should only be logged once per run', async () => {
            ...
            const maxCrawlDepthMessages = infoSpy.mock.calls.filter((call) => typeof call[0] === 'string' && call[0].includes('maxCrawlDepth'));
        ...
        test('enqueueLinks should clamp an explicit limit to the remaining maxRequestsPerCrawl budget', async () => {
            ...
            expect(infoSpy).toHaveBeenCalledWith(expect.stringContaining('due to the remaining maxRequestsPerCrawl budget of 2'));
        });
        test('enqueueLinks should keep reporting skipped requests when the user passes onSkippedRequest', async () => {
            ...
            for (const mock of [crawlerOnSkippedRequest, userOnSkippedRequest]) {
                expect(mock.mock.calls.map((call) => call[0]).sort((a, b) => a.url.localeCompare(b.url))).toEqual(skipped);
            }
        });
    });
    describe('transactional storage', () => {
            ...
            const observingManager = new Proxy(realManager, {
                get(target, prop, receiver) {
                    if (prop === 'fetchNextRequest') {
                        return async (...args) => {
                            fetches += 1;
                            if (currentStorageTransaction()?.isActive)
                                fetchedInsideTransaction = true;
                            return target.fetchNextRequest(...args);
                        };
                    }
        ...
        test('onSkippedRequest bookkeeping survives an in-pipeline robots.txt skip', async () => {
            ...
            vitest.spyOn(crawler, 'isAllowedBasedOnRobotsTxtFile').mockResolvedValue(false);
            await crawler.run();
        ...
        test('an unrecognized write policy is rejected instead of falling back to the default', () => {
            const make = (transactionalStorage) => new BasicCrawler({ requestHandler: async () => { }, transactionalStorage });
    ...
    describe('addRequests input validation', () => {
        test('should throw error when passed a non-iterable value', async () => {
            ...
            await expect(crawler.addRequests(new WeakSet())).rejects.toThrow('Expected an iterable or async iterable, got weakset');
        });
    });
    describe('Dataset helpers, crawler parallelism', () => {
        const payload = [{ foo: 'bar', baz: 123 }];
        const getPayload = (id) => [{ foo: id }];
    ...
    describe('schema validation on add', () => {
        ...
        test('crawler.addRequests rejects userData that does not match the label schema', async () => {
            ...
            await expect(crawler.addRequests([
        ...
        test('crawler.run rejects userData that does not match the label schema', async () => {
            const crawler = makeCrawler();
            await expect(crawler.run([{ url: 'https://example.com/a', label: 'DETAIL', userData: { id: 123 } }])).rejects.toThrow(RequestValidationError);
        });

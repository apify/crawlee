...
describe('Statistics', () => {
    ...
    const persistStateKey = (statistics) => `CRAWLEE_CRAWLER_STATISTICS_${statistics.id}`;
    let stats;
    let store;
    beforeAll(async () => {
    ...
    describe('persist state', () => {
        ...
        test('should restore an Infinity minimum duration written out as null', async () => {
            await stats.startCapturing();
            await stats.stopCapturing();
            const record = await store.getValue(persistStateKey(stats));
        ...
        test('should start from scratch on a record it cannot make sense of', async () => {
            ...
            await restored.startCapturing();
            expect(warningSpy).toHaveBeenCalledWith(expect.stringContaining('starting the statistics from scratch'), expect.objectContaining({ persistStateKey: persistStateKey(stats) }));
        ...
        test('should write the record when the run is too short to have a rate', async () => {
            ...
            expect(await store.getValue(persistStateKey(stats))).toMatchObject({
                requestsFinished: 1,
                requestsFailed: 1,
    ...
    test('should regularly log stats', async () => {
        ...
        infoSpy.mockImplementation((message, data) => {
    ...
    describe('custom state fields', () => {
        ...
        const persistCustomState = async (id) => {
        ...
        test('should expose the custom fields and restore their defaults on reset', () => {
            const stats = new Statistics({
                stateExtension: { defaultState: { productsFound: 0, seenPerDomain: {} } },
            });
        ...
        test('should reject a custom field that collides with a built-in one', () => {
            expect(() => new Statistics({ stateExtension: { defaultState: { requestsFinished: 999 } } })).toThrow(/`requestsFinished` collides with a built-in one/);
        });
        ...
        describe('with a deserialize conversion', () => {
            ...
            test('should throw when the defaults cannot be derived', () => {
                expect(() => new Statistics({ stateExtension: { deserialize: z.object({ productsFound: z.number() }) } })).toThrow(/Could not derive the default values .* give every field a default/s);
            });
            ...
            test('should start only the custom fields from scratch on a record it rejects', async () => {
                ...
                const record = (await store.getValue(`CRAWLEE_CRAWLER_STATISTICS_corrupt-custom-stats`));
                ...
                await stats.startCapturing();
                expect(warningSpy).toHaveBeenCalledWith(expect.stringContaining('starting those from scratch'), expect.objectContaining({ persistStateKey: 'CRAWLEE_CRAWLER_STATISTICS_corrupt-custom-stats' }));
            ...
            test('should take the record at its word without one', async () => {
                ...
                const record = (await store.getValue(`CRAWLEE_CRAWLER_STATISTICS_trusted-stats`));
        ...
        test('should round-trip a field that is not JSON through a serialize/deserialize pair', async () => {
            const stateExtension = {
                ...
                serialize: ({ lastSeenAt }) => ({ lastSeenAt: lastSeenAt.toISOString() }),
            };
            const stats = new Statistics({ id: 'date-stats', stateExtension });
            await stats.startCapturing();
            stats.state.lastSeenAt = new Date('2020-01-01T00:00:00.000Z');
            await stats.stopCapturing();
            expect(await store.getValue('CRAWLEE_CRAWLER_STATISTICS_date-stats')).toMatchObject({

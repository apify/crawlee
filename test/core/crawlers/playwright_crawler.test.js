...
import { createPlaywrightRouter, PlaywrightCrawler, playwrightBrowserPool, RequestList, RequestValidationError, } from '@crawlee/playwright';
...
describe('PlaywrightCrawler', () => {
    ...
    beforeAll(async () => {
        ...
        app.get('/page-with-download', (_req, res) => {
            res.status(200).send(`<html><body><a id="download-link" href="/download-file" download="hello.txt">download</a></body></html>`);
        });
    ...
    describe('should work', () => {
        ...
        test.each(['chromium', 'firefox'])('with %s', async (browser) => {
            ...
            const requestHandler = async ({ page, request, response, useState }) => {
                await useState([]);
            ...
            expect(playwrightCrawler.concurrencySystem.minConcurrency).toBe(1);
    ...
    describe('playwrightBrowserPool', () => {
        ...
        test('turns off fingerprint injection when a custom userAgent is given', () => {
            expect(playwrightBrowserPool({ launchContext: { userAgent: 'Definitely Not A Crawler' } }).useFingerprints).toBe(false);
        });
        test('is rejected by the crawler alongside the options that would configure its own pool', () => {
            const browserPool = playwrightBrowserPool();
            expect(() => new PlaywrightCrawler({ browserPool, headless: false })).toThrow('PlaywrightCrawler: `headless` cannot be combined with `browserPool`');
            expect(() => new PlaywrightCrawler({ browserPool, launchContext: { launcher: playwright.firefox } })).toThrow('PlaywrightCrawler: `launchContext` cannot be combined with `browserPool`');
            expect(() => new PlaywrightCrawler({ browserPool })).not.toThrow();
        });
    });
    test.each([{ useIncognitoPages: true }, { useIncognitoPages: false }])('should apply launchOptions with useIncognitoPages: $useIncognitoPages', async ({ useIncognitoPages }) => {

import { BaseCrawleeLogger, Dataset, EventType, KeyValueStore, MemoryStorageBackend, serviceLocator, Statistics, } from '@crawlee/core';
import { AdaptivePlaywrightCrawler, adaptivePlaywrightCrawlerStatisticState, BasicCrawler, createAdaptivePlaywrightRouter, fullResultComparator, RenderingTypePredictor, RequestList, RequestQueue, RequestValidationError, } from '@crawlee/playwright';
...
describe('AdaptivePlaywrightCrawler', () => {
    ...
    // Test setup helpers
    const makeOneshotCrawler = async (options, sources) => new AdaptivePlaywrightCrawler({
    ...
    const makeRiggedRenderingTypePredictor = (prediction) => ({
        predict: vi.fn((_request) => prediction),
        storeResult: vi.fn((_request, _renderingType) => { }),
    });
    test.each([
        ['/static', 'static'],
        ['/dynamic', 'clientOnly'],
    ])('extendContext is visible to pre/post-navigation hooks and the request handler (%s)', async (path, renderingType) => {
    ...
    describe('should detect page rendering type', () => {
        test.each([
            ['/static', 'static'],
            ['/dynamic', 'clientOnly'],
        ])('for %s', async (path, expectedType) => {
            const renderingTypePredictor = makeRiggedRenderingTypePredictor({
                detectionProbabilityRecommendation: 1,
                renderingType: 'clientOnly',
            });
            const url = new URL(`http://${HOSTNAME}:${port}${path}`);
            const requestHandler = vi.fn(async ({ pushData, parseWithCheerio }) => {
    ...
    describe('querySelector and querySelectorAll', () => {
        test.each([
            ['/static', 'static'],
            ['/dynamic', 'clientOnly'],
        ])('return first vs. all matched elements (%s)', async (path, renderingType) => {
            const renderingTypePredictor = makeRiggedRenderingTypePredictor({
                detectionProbabilityRecommendation: 0,
                renderingType,
            });
            const url = new URL(`http://${HOSTNAME}:${port}${path}`);
            const requestHandler = vi.fn(async ({ pushData, querySelector, querySelectorAll }) => {
    ...
    test.each([['static'], ['clientOnly']])('should replay request handler logs (%s)', async (renderingType) => {
        const renderingTypePredictor = makeRiggedRenderingTypePredictor({
            detectionProbabilityRecommendation: 0,
            renderingType,
        });
        ...
        const messages = [];
        const requestHandler = vi.fn(async ({ log }) => {
    ...
    test('should retry with browser if result checker returns false', async () => {
        ...
        const url = new URL(`http://${HOSTNAME}:${port}/dynamic`);
        const requestHandler = vi.fn(async ({ pushData, querySelector }) => {
            await pushData({
                heading: (await querySelector('h1')).text(),
            });
        });
        const resultChecker = vi.fn((result) => result.datasetItems.length > 0 &&
            result.datasetItems.every(({ item }) => item.heading?.length > 0));
        const crawler = await makeOneshotCrawler({
            requestHandler,
            renderingTypePredictor,
            resultChecker,
    ...
    test.each([['static'], ['clientOnly']])('crawlingContext.addRequests() should add requests correctly (%s)', async (renderingType) => {
        const renderingTypePredictor = makeRiggedRenderingTypePredictor({
            detectionProbabilityRecommendation: 0,
            renderingType,
        });
        ...
        const requestHandler = async (context) => {
            ...
            if (isStartUrl)
                await context.addRequests([`${url}/1`]);
            else
                requestContext = context;
        };
        const crawler = await makeOneshotCrawler({ requestHandler, renderingTypePredictor, maxRequestsPerCrawl: 10 }, []);
    ...
    describe('should enqueue links correctly', () => {
        test.each([
            ['/static', 'static'],
            ['/dynamic', 'clientOnly'],
        ])('for %s', async (path, renderingType) => {
            const renderingTypePredictor = makeRiggedRenderingTypePredictor({
                detectionProbabilityRecommendation: 0,
                renderingType,
            });
            const url = new URL(`http://${HOSTNAME}:${port}${path}`);
            const enqueuedRequests = [];
            const requestHandler = vi.fn(async ({ enqueueLinks, request }) => {
    ...
    test.each([['static'], ['clientOnly']])('should respect the strategy option for enqueueLinks (%s)', async (renderingType) => {
        const renderingTypePredictor = makeRiggedRenderingTypePredictor({
            detectionProbabilityRecommendation: 0,
            renderingType,
        });
    ...
    test('should persist crawler state', async () => {
        ...
        const requestHandler = vi.fn(async ({ useState }) => {
    ...
    test('should persist key-value store changes', async () => {
        ...
        const requestHandler = vi.fn(async ({ request, getKeyValueStore }) => {
    ...
    test('should reject transactionalStorage: false at construction', () => {
        expect(() => new AdaptivePlaywrightCrawler({
    ...
    test('should persist RenderingTypePredictor state on PERSIST_STATE events', async () => {
        const requestHandler = vi.fn(async ({ pushData }) => {
    ...
    describe('rendering type predictor lifecycle', () => {
        const requestHandler = async ({ pushData }) => {
            await pushData({ content: 'test data' });
        };
        test('initializes the default predictor it built itself', async () => {
            ...
            await sleep(100);
            const store = await KeyValueStore.open();
            await expect(store.getValue('rendering-type-predictor-state')).resolves.not.toBeNull();
        });
    ...
    test('validates userData against the router schema when adding requests', async () => {
        ...
        await expect(crawler.addRequests([{ url: 'https://example.com/a', label: 'DETAIL', userData: { id: 123 } }])).rejects.toThrow(RequestValidationError);
    });

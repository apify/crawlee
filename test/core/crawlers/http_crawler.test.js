...
import { ConcurrencySystem, HttpCrawler, PersistentRateLimitError, RequestQueue, SessionPool, ThrottlingRequestManager, } from '@crawlee/http';
...
test('concurrency shortcuts coexist with the HTTP-optimized defaults', async () => {
    ...
    expect(crawler.concurrencySystem.maxConcurrency).toBe(5);
...
test('should ignore http error status codes set by user', async () => {
    ...
    expect(crawler.concurrencySystem.minConcurrency).toBe(2);
...
test('`keepAlive` outlives a domain that never stops rate-limiting', async () => {
    ...
    const outcome = await Promise.race([
        running.then(() => 'shut down', () => 'threw'),
        sleep(3000).then(() => 'still running'),
    ]);

import { ContextPipeline, ContextPipelineCleanupError, ContextPipelineInitializationError, ContextPipelineInterruptedError, RequestHandlerError, } from '@crawlee/core';
...
describe('ContextPipeline', () => {
    ...
    it('should wrap pipeline initialization errors', async () => {
        ...
        const consumer = vi.fn();
        await expect(pipeline.call(context, consumer)).rejects.toThrow(expect.objectContaining({
    ...
    describe('chain', () => {
        it('should run middlewares from both pipelines in order', async () => {
            const first = ContextPipeline.create().compose({
                action: async (ctx) => ({ b: ctx.a + 1 }),
            });
            const second = ContextPipeline.create().compose({

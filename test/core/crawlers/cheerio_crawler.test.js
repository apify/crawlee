...
import { CheerioCrawler, createCheerioRouter, EnqueueStrategy, mergeCookies, ProxyConfiguration, Request, RequestList, Session, } from '@crawlee/cheerio';
...
async function getRequestListForMock(mockData, pathName = 'special/mock') {
...
describe('CheerioCrawler', () => {
    ...
    test('should work', async () => {
        ...
        const processed = [];
        const failed = [];
        const requestHandler = ({ $, body, request }) => {
        ...
        expect(cheerioCrawler.concurrencySystem.minConcurrency).toBe(2);
    ...
    test('forwards ignoreTlsErrors to the http client', async () => {
        ...
        const capturingClient = Object.assign(Object.create(BaseHttpClient.prototype), {
    ...
    describe('should ensure text/html Content-Type', () => {
        ...
        describe('by throwing', () => {
            ...
            test('when invalid Content-Type header is received', async () => {
                ...
                errorMessages.forEach((msg) => expect(msg).toMatch(' Content-Type text/plain, but only text/html, text/xml, application/xhtml+xml, application/xml, application/json are allowed.' +
                    ' Skipping resource.'));
            });
    ...
    describe('proxy', () => {
        ...
        test('requestHandler should expose the proxyInfo object with sessions correctly', async () => {
            ...
            const proxies = [];
            const sessions = [];
            const requestHandler = ({ session, proxyInfo }) => {
        ...
        test('proxy rotation logs the original proxy error', async () => {
            ...
            const spy = vitest.spyOn(crawler.log, 'warning').mockImplementation(() => { });
            await crawler.run([serverAddress]);
    ...
    describe('SessionPool', () => {
        ...
        test('should markBad sessions after request timeout', async () => {
            ...
            await cheerioCrawler.run();
            const { sessions } = await cheerioCrawler.sessionPool.getState();
        ...
        test('should retire session on "blocked" status codes', async () => {
            for (const code of [401, 403, 429]) {
                ...
                await crawler.run();
                const { sessions: poolSessions } = await crawler.sessionPool.getState();
        ...
        test('mergeCookies()', async () => {
            ...
            expect(warningSpy).toBeCalledWith(`Found cookies with similar name during cookie merging: 'Other' and 'other'`);
        ...
        test('sendRequest and main request should share the same session cookie jar', async () => {
            ...
            const crawler = new CheerioCrawler({
                ...
                requestHandler: async ({ json, sendRequest }) => {
                    responses.push(json);
                    const sendRequestJson = await sendRequest({ url: `${serverAddress}/special/get-cookies` }).then(async (response) => response.json());
                    responses.push(sendRequestJson);
                },
            });
            await crawler.run();
        ...
        test('sendRequest should respect Cookie header override', async () => {
            ...
            const crawler = new CheerioCrawler({
                ...
                requestHandler: async ({ sendRequest }) => {
                    ...
                    responses.push((await withHeader.json()));
                    const withoutOverride = await sendRequest({
                        url: `${serverAddress}/special/get-cookies`,
                    });
                    responses.push((await withoutOverride.json()));
                },
            });
            await crawler.run();
            expect(responses).toHaveLength(2);
        ...
        test('sendRequest should respect cookieJar override', async () => {
            ...
            const crawler = new CheerioCrawler({
                ...
                requestHandler: async ({ sendRequest }) => {
                    ...
                    const withJar = await sendRequest({ url: `${serverAddress}/special/get-cookies` }, { cookieJar: customJar });
                    responses.push((await withJar.json()));
    ...
    describe('Crawling context', () => {
        ...
        test('uses correct crawling context', async () => {
            ...
            const preNavigationHook = (crawlingContext) => {
            ...
            const requestHandler = (crawlingContext) => {
            ...
            const failedRequestHandler = (crawlingContext, error) => {
                expect(crawlingContext === prepareCrawlingContext).toEqual(true);
                expect(crawlingContext.request).toBeInstanceOf(Request);
                expect(crawlingContext.session).toBeInstanceOf(Session);
                expect(typeof crawlingContext.$).toBe('function');

import { BROWSER_POOL_EVENTS, BrowserPool as BrowserPoolClass, OperatingSystemsName, PuppeteerPlugin, RemoteBrowserPool, } from '@crawlee/browser-pool';
import { bindMethodsToServiceLocator, BLOCKED_STATUS_CODES, MemoryStorageBackend, serviceLocator, ServiceLocator, SessionPool, } from '@crawlee/core';
...
describe('BrowserCrawler', () => {
    ...
    // Runs inside the `aroundEach` scope, after the global setup's `serviceLocator.reset()`,
    // so it can't be clobbered by it.
    beforeEach(() => {
    ...
    test.concurrent('should work', async () => {
        ...
        const requestHandler = async ({ page, request, response }) => {
        ...
        expect(browserCrawler.concurrencySystem.minConcurrency).toBe(1);
    ...
    test.concurrent('should teardown browser pool', async () => {
        ...
        const ownedPool = browserCrawler.browserPool;
    ...
    test.concurrent('builds and owns a RemoteBrowserPool from the remoteBrowser option', async () => {
        ...
        expect(crawler.browserPool.maxOpenBrowsers).toBe(2);
    ...
    describe('Crawling context', () => {
        ...
        test('uses correct crawling context', async () => {
            ...
            try {
                ...
                const failedRequestHandler = async (crawlingContext, error) => {

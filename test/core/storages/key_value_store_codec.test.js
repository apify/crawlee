...
describe('key_value_store_codec', () => {
    describe('serializeValue()', () => {
        ...
        test('"Object is too large" remap', () => {
            ...
            expect(() => serializeValue(tooLong)).toThrow('The "value" parameter cannot be stringified to JSON: Object is too large');
        });
        test('circular structure error is surfaced', () => {
            const obj = {};
            obj.self = obj;
            expect(() => serializeValue(obj)).toThrow('The "value" parameter cannot be stringified to JSON: Converting circular structure to JSON');
        });
        test('undefined-after-stringify guard', () => {
            expect(() => serializeValue(undefined)).toThrow('The "value" parameter was stringified to JSON and returned undefined.');
        });
    });
    describe('parseValue()', () => {
    ...
    describe('round-trips', () => {
        test('object → json → object', () => {
            ...
            expect(parseValue(Buffer.from(value), contentType)).toEqual(original);
        });
        test('string → text → string', () => {
            const original = 'hello world';
            const { value, contentType } = serializeValue(original, 'text/plain; charset=utf-8');
            expect(parseValue(Buffer.from(value), contentType)).toBe(original);
        });
        test('Buffer → octet-stream → Buffer', () => {
            ...
            expect(parseValue(value, contentType)).toBe(original);
        });
        ...
        test('no content type: Buffer round-trips as a Buffer (not JSON-mangled)', () => {
            ...
            const { value, contentType } = serializeValue(original);
            const parsed = parseValue(value, contentType);

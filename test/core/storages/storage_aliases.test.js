...
describe('storage aliases', () => {
    ...
    describe('name/alias conflict detection', () => {
        test('should throw when opening alias that conflicts with existing named dataset', async () => {
            ...
            await expect(Dataset.open({ alias: 'shared-name' })).rejects.toThrow(/Cannot open storage with alias "shared-name" because a named storage with the same identifier already exists/);
        });
        test('should throw when opening named dataset that conflicts with existing alias', async () => {
            ...
            await expect(Dataset.open({ name: 'shared-name' })).rejects.toThrow(/Cannot open storage with name "shared-name" because an alias storage with the same identifier already exists\. If you meant to open the alias storage, use \{ alias: "shared-name" \} instead\./);
        });
        test('should throw when opening alias that conflicts with existing named KVS', async () => {
            ...
            await expect(KeyValueStore.open({ alias: 'shared-kvs' })).rejects.toThrow(/Cannot open storage with alias "shared-kvs" because a named storage with the same identifier already exists/);
        });
    ...
    describe('string identifier vs alias conflict', () => {
        test('should throw when opening a string identifier that matches an existing alias', async () => {
            ...
            await expect(Dataset.open('asdf')).rejects.toThrow(/Cannot open storage with name "asdf" because an alias storage with the same identifier already exists\. If you meant to open the alias storage, use \{ alias: "asdf" \} instead\./);
        });
        test('should throw when opening an alias that matches an existing string-opened storage', async () => {
            await Dataset.open('asdf');
            await expect(Dataset.open({ alias: 'asdf' })).rejects.toThrow(/Cannot open storage with alias "asdf" because a named storage with the same identifier already exists/);
        });
    });
    describe('string identifier vs alias conflict (persistent storage)', () => {
        ...
        test('should throw when opening a string identifier that matches an existing alias on disk', async () => {
            ...
            await expect(Dataset.open('on-disk')).rejects.toThrow(/Cannot open storage with name "on-disk" because an alias storage with the same identifier already exists\. If you meant to open the alias storage, use \{ alias: "on-disk" \} instead\./);
        });
    ...
    describe('resolveStorageIdentifier', () => {
        ...
        describe('at the backend level', () => {
            ...
            test.each([
                ['MemoryStorageBackend', () => new MemoryStorageBackend()],
                [
                    'FileSystemStorageBackend',
                    () => new FileSystemStorageBackend({ localDataDirectory: localStorageDir }),
                ],
            ])('%s opens the same default storage for the reserved alias and for no identifier', async (_backendName, createBackend) => {

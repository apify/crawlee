...
const createMockHttpClient = () => Object.assign(Object.create(BaseHttpClient.prototype), {
    sendRequest: vitest.fn(async (_request, _options) => new Response()),
...
describe('RequestQueue remote', () => {
    ...
    describe('setExpectedRequestProcessingTimeSecs', () => {
        test('forwards the value to the client, but only ever raises it', async () => {
            ...
            const spy = vitest.fn();
            queue.backend.setExpectedRequestProcessingTimeSecs = spy;
...
describe('RequestQueue with requestsFromUrl', () => {
    ...
    test('should correctly load list from hosted files in correct order', async () => {
        const spy = vitest.spyOn(RequestQueue.prototype, 'downloadListOfUrls');
        const list1 = ['https://example.com', 'https://google.com', 'https://wired.com'];
        const list2 = ['https://another.com', 'https://page.com'];
        spy.mockImplementationOnce(() => new Promise((resolve) => setTimeout(() => resolve(list1), 100)));
...
describe('RequestQueue (request lifecycle)', () => {
    ...
    function calculateHistogram(requests) {
        const histogram = [];
    ...
    test('`fetchNextRequest` order respects `forefront` enqueues', async () => {
        ...
        expect(retrievedUrls.map((x) => new URL(x).pathname)).toEqual(Array.from({ length: 28 }, (_, i) => `/${i + 1}`));
    });
...
describe('RequestQueue background batches', () => {
    ...
    test('a failing background batch rejects instead of hanging, and stops blocking isFinished', async () => {
        ...
        const result = await queue.addRequestsBatched([{ url: 'https://example.com/1' }, { url: 'https://example.com/2' }], { batchSize: 1, waitBetweenBatchesMillis: 0 });

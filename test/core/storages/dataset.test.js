import { ArgumentValidationError, assertJsonSerializable, Dataset, KeyValueStore, MemoryStorageBackend, serviceLocator, } from '@crawlee/core';
...
describe('dataset', () => {
    ...
    describe('remote', () => {
        ...
        test('getData() should work', async () => {
            ...
            await expect(dataset.getData()).rejects.toThrow('dataset.getData(): The response is too large for parsing. You can fix this by lowering the "limit" option.');
        });
    ...
    describe('pushData', () => {
        test('throws on invalid args', async () => {
            ...
            await expect(dataset.pushData(() => { })).rejects.toThrow('Data item at index 0 is not an object. You can push only objects into a dataset.');
            const circularObj = {};

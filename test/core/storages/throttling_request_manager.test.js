import { KeyValueStore, MemoryStorageBackend, PersistentRateLimitError, RequestQueue, serviceLocator, ThrottlingRequestManager, withStorageTransaction, } from '@crawlee/core';
...
describe('ThrottlingRequestManager', () => {
    ...
    function domainState(manager, domain) {
        return manager.domainStates.get(domain);
    }
    /** Models the crawler's task loop: poll, and idle while the manager reports itself empty. */
    async function pollForNextRequest(manager, timeoutMs = 5000) {
    ...
    test('addRequestsBatched keeps isFinished false while background batches are landing', async () => {
        ...
        const result = await manager.addRequestsBatched([{ url: 'https://other.com/1' }, { url: 'https://other.com/2' }], { batchSize: 1, waitBetweenBatchesMillis: 0 });
    ...
    test('warns that requestsFromUrl sources cannot be domain-routed', async () => {
        ...
        const warning = vitest.spyOn(manager.log, 'warning').mockImplementation(() => { });
        await manager.addRequestsBatched([
    ...
    describe('stall detection', () => {
        const stallingManager = async () => new ThrottlingRequestManager({
            inner: await createQueue(),
            domains: ['example.com'],
            baseDelaySecs: 0.01,
    ...
    describe("domains: 'all'", () => {
        ...
        test('a batch that overflows maxThrottledDomains still stores the requests that fit', async () => {
            ...
            const error = await manager.addRequestsBatched(urls).catch((e) => e);
            expect(error).toBeInstanceOf(Error);
            // The other 49 domains that did not fit either are named, not just the first one to overflow.
            expect(error.message).toMatch(/49 other new domain\(s\)/);

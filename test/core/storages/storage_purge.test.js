...
const requestOf = (url) => ({ url, uniqueKey: url });
const pendingCount = async (queue) => (await queue.getMetadata()).pendingRequestCount;
const readInput = async (store) => (await store.getValue('INPUT'))?.value.toString();
...
describe.each([
    ...
    [
        ...
        () => new FileSystemStorageBackend({ localDataDirectory: temporaryDirectory() }),
    ],

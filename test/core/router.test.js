...
import { CheerioCrawler, createCheerioRouter, createPlaywrightRouter, createPuppeteerRouter, PlaywrightCrawler, PuppeteerCrawler, } from 'crawlee';
import { z } from 'zod';
describe('Router', () => {
    test('should be callable and route based on the label', async () => {
        ...
        await router({ request: { loadedUrl: 'https://example.com/A', label: 'A' }, log });
        await router({ request: { loadedUrl: 'https://example.com/A', label: 'A' }, log });
        await router({ request: { loadedUrl: 'https://example.com/C', label: 'C' }, log });
        await router({ request: { loadedUrl: 'https://example.com/A', label: 'A' }, log });
        await router({ request: { loadedUrl: 'https://example.com/B', label: 'B' }, log });
        await router({ request: { loadedUrl: 'https://example.com/B', label: 'B' }, log });
        await router({ request: { loadedUrl: 'https://example.com/' }, log });
        await router({ request: { loadedUrl: 'https://example.com/A', label: 'A' }, log });
        await router({ request: { loadedUrl: 'https://example.com/C', label: 'C' }, log });
        await router({ request: { loadedUrl: 'https://example.com/C', label: 'C' }, log });
        await router({ request: { loadedUrl: 'https://example.com/D', label: 'D' }, log });
        await router({ request: { loadedUrl: 'https://example.com/C', label: 'C' }, log });
        expect(logs).toEqual([
    ...
    test('validation', async () => {
        ...
        const log = { info: vitest.fn(), warn: vitest.fn(), debug: vitest.fn() };
        await expect(router({ request: { loadedUrl: 'https://example.com/C', label: 'C' }, log })).rejects.toThrow(MissingRouteError);
        router.addDefaultHandler(async (ctx) => { });
    ...
    test('addHandler accepts userdata generic', async () => {
        ...
        router.addHandler('1', (ctx) => {
            testType(ctx.request.userData.foo);
        });
        router.addHandler('2', (ctx) => {
            testType(ctx.request.userData.foo);
        });
    ...
    test('addHandler infers userData from a declared route map', async () => {
        ...
        router.addHandler('PRODUCT', (ctx) => {
            testType(ctx.request.userData.sku);
            testType(ctx.request.userData.price);
        });
        router.addHandler('CATEGORY', (ctx) => {
            testType(ctx.request.userData.categoryId);
        });
    ...
    test('factory keeps the legacy flat-userData generic working (backwards compatibility)', async () => {
        ...
        router.addHandler('anyLabel', (ctx) => {
            testType(ctx.request.userData.token);
        });
    ...
    test('schema map infers userData types and validates them at dispatch', async () => {
        ...
        await router({ request: validRequest, log });
    ...
    test('a defaultRoute schema also types the default handler userData', () => {
        ...
        const typeOnly = () => {
            ...
            router.addDefaultHandler(({ request }) => {
                ...
                testType(request.userData.page);
            ...
            plain.addDefaultHandler(({ request }) => {
                testType(request.userData.anything);
            });
        };

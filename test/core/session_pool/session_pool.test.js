...
describe('SessionPool - testing session pool', () => {
    ...
    describe('should retrieve session', () => {
        test('should retrieve session with correct shape', async () => {
            ...
            expect(session.expiresAt.getTime() - session.createdAt.getTime()).toEqual(sessionPool.sessionOptions.maxAgeSecs * 1000);
    ...
    test('should persist state and recreate it from storage', async () => {
        ...
        expect((await sessionPool.getState()).sessions).toHaveLength((await loadedSessionPool.getState()).sessions.length);
    ...
    test('should createSessionFunction work', async () => {
        let isCalled = false;
        let receivedOptions;
        const createSessionFunction = (opts) => {
            isCalled = true;
    ...
    describe('sessionReuseStrategy', () => {
        ...
        test.each(['random', 'round-robin'])('%s should evict a retired session from a full pool and replenish', async (strategy) => {

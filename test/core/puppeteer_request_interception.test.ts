...
describe('utils.puppeteer.addInterceptRequestHandler|removeInterceptRequestHandler()', () => {
    test('should allow multiple handlers', async () => {
        ...
        expect(allUrls).toEqual(expect.arrayContaining([`${serverAddress}/script.js`, `${serverAddress}/style.css`, `${serverAddress}/image.png`]));
...
describe('utils.puppeteer.removeInterceptRequestHandler()', () => {
    test('works', async () => {
        ...
        try {
            ...
            // Try it once again.
            await page.goto(`${serverAddress}/special/resources`, { waitUntil: 'networkidle0' });
            expect(loadedUrls).toEqual(expect.arrayContaining([`${serverAddress}/style.css`, `${serverAddress}/style.css`]));

...
import { getContainerCpuUsage, getCpuPeriod, getCpuQuota, getCurrentCpuTicks, getCurrentCpuTicksV2, getSystemCpuUsage, sampleCpuUsage, } from '../../../packages/core/src/system-info/cpu-info.js';
...
vitest.mock('node:fs/promises', async (importActual) => {
    const originalFs = await importActual();
    return {
        ...originalFs,
...
function resetCpuTicksBaseline() {
    const cpusMock = vitest
        .spyOn(os, 'cpus')
        .mockReturnValue([{ times: { user: 0, nice: 0, sys: 0, idle: 0, irq: 0 } }]);
    getCurrentCpuTicks();
...
describe('getCurrentCpuTicks()', () => {
    ...
    test('measures load between samples, not cumulatively since boot', () => {
        ...
        const firstMock = vitest
            .spyOn(os, 'cpus')
            .mockReturnValue([{ times: { user: 100, nice: 0, sys: 100, idle: 200, irq: 0 } }]);
        getCurrentCpuTicks();
        ...
        const secondMock = vitest
            .spyOn(os, 'cpus')
            .mockReturnValue([{ times: { user: 175, nice: 0, sys: 175, idle: 250, irq: 0 } }]);
        const load = getCurrentCpuTicks();
...
describe('getCpuInfo()', () => {
    test('returns bare metal cpu ticks in AWS Lambda environment', async () => {
        ...
        const cpusMock = vitest
            .spyOn(os, 'cpus')
            .mockReturnValue([{ times: { user: 100, nice: 0, sys: 50, idle: 50, irq: 0 } }]);
    ...
    test('returns bare metal cpu ticks when not containerized', async () => {
        const cpusMock = vitest
            .spyOn(os, 'cpus')
            .mockReturnValue([{ times: { user: 200, nice: 0, sys: 100, idle: 100, irq: 0 } }]);
        const load = await getCurrentCpuTicksV2();
    ...
    test('logs warningOnce when containerized but cgroups not available', async () => {
        ...
        const cpusMock = vitest
            .spyOn(os, 'cpus')
            .mockReturnValue([{ times: { user: 100, nice: 0, sys: 50, idle: 50, irq: 0 } }]);
        const logger = { warningOnce: vitest.fn(), warning: vitest.fn() };
    ...
    test('returns bare metal cpu ticks when containerized but no cgroup quota', async () => {
        ...
        const cpusMock = vitest
            .spyOn(os, 'cpus')
            .mockReturnValue([{ times: { user: 300, nice: 0, sys: 150, idle: 150, irq: 0 } }]);
        const result = await getCurrentCpuTicksV2({ containerized: true });

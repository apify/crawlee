 ...
 */
async function makePool(poolOptions, concurrencyOptions = {}) {
...
describe('AutoscaledPool', () => {
    ...
    describe('should scale correctly', () => {
        ...
        test('works with low values', () => {
            ...
            pool.system.tryRegisterTaskStart();
            pool.system.tryRegisterTaskStart();
            // @ts-expect-error Calling private method on the governor
            pool.system.autoscale(cb);
        ...
        test('works with high values', () => {
            ...
            const targetConcurrency = Math.floor(pool.desiredConcurrency * pool.system.desiredConcurrencyRatio);
    ...
    describe('should throw', () => {
        ...
        test('when some of the promises throws', async () => {
            ...
            const runTaskFunction = async () => {
                ...
                await sleep(1);
                if (counter > 20)
                    throw new Error('some-promise-error');
            };
            const pool = await makePool({
                runTaskFunction,
        ...
        test('when isTaskReadyFunction throws', async () => {
            ...
            const pool = await makePool({
                ...
                isTaskReadyFunction: async () => {
                    if (count > 1)
                        throw new Error('some-ready-error');
                    else
                        return true;
                },
            }, { maxConcurrency: 1 });
    ...
    test('should not handle tasks added after isFinishedFunction returned true', async () => {
        ...
        const pool = await makePool({
            runTaskFunction: async () => sleep(1).then(() => {
                count++;
            }),
    ...
    describe('custom load signals', () => {
        ...
        function createFakeLoadSignal(name, { overloadedRatio = 0.3, isOverloaded = false } = {}) {
            const now = Date.now();

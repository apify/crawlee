...
import { MemoryStorageBackend, serviceLocator, SitemapRequestLoader } from '@crawlee/core';
...
describe('SitemapRequestLoader', () => {
    ...
    async function collectUrls(list) {
        const urls = [];
        for await (const request of list) {
    ...
    test('`enqueueStrategy: all` keeps cross-host URL entries', async () => {
        ...
        expect(new Set(await collectUrls(list))).toEqual(new Set([`${url}/same-host-page`, `${cross}/cross-host-page`]));
    });
    ...
    test('the selected enqueue strategy is stamped onto emitted requests', async () => {
        ...
        expect(request.enqueueStrategy).toBe('same-hostname');
    });

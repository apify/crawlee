...
describe('StagehandController', () => {
    ...
    test('should set cookies (stubbed - Stagehand v3 limitation)', async () => {
        ...
        await expect(controller._setCookies(mockPage, cookies)).resolves.toBeUndefined();
    });
    test('should get cookies (stubbed - Stagehand v3 limitation)', async () => {
        ...
        const cookies = await controller._getCookies(mockPage);
    ...
    test('should handle close errors gracefully', async () => {
        ...
        // Should not throw
        await expect(controller._close()).resolves.toBeUndefined();
    ...
    test('should handle kill errors gracefully', async () => {
        ...
        controller.browser = mockBrowser;
        // Should not throw
        await expect(controller._kill()).resolves.toBeUndefined();
    });
    describe('_newPage', () => {
        let mockCdpSession;
        let mockPage;
        ...
        test('should not return the page before Stagehand registers it', async () => {
            ...
            let attempts = 0;
            mockStagehand.context.resolvePageByMainFrameId = vi.fn(() => ++attempts < 3 ? undefined : { v3Page: true });
            const page = await createController()._newPage();
        ...
        test('should detach the CDP session used to read the main frame id', async () => {
            ...
            await createController()._newPage();
        ...
        test('should throw when Stagehand never registers the page', async () => {
            ...
            await expect(controller.waitForStagehandToRegisterPage(mockPage, 100)).rejects.toThrow('Stagehand did not register the page within 100ms');
        });
        test('should close the page when Stagehand never registers it', async () => {
            const controller = createController();
            vi.spyOn(controller, 'waitForStagehandToRegisterPage').mockRejectedValue(new Error('not registered'));
            await expect(controller._newPage()).rejects.toThrow('Failed to create new page: not registered');

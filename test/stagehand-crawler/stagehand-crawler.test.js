import { z } from 'zod';
import { createStagehandRouter, stagehandBrowserPool, StagehandCrawler, } from '../../packages/stagehand-crawler/src/index.js';
...
describe('StagehandCrawler', () => {
    ...
    test('forwards headless to the browser plugin', () => {
        ...
        expect(crawler.browserPool.browserPlugins[0].launchOptions).toMatchObject({
            headless: false,
        });
    });
    test('rejects the options that would configure its own pool when a browserPool is passed in', () => {
        ...
        const browserPool = stagehandBrowserPool();
        expect(() => new StagehandCrawler({ browserPool, stagehandOptions: { env: 'LOCAL' } })).toThrow('StagehandCrawler: `stagehandOptions` cannot be combined with `browserPool`');
...
describe('enhancePageWithStagehand', () => {
    ...
    test('extract() should wrap errors', async () => {
        ...
        const enhancedPage = enhancePageWithStagehand(mockPage, mockStagehand);
        await expect(enhancedPage.extract('Get data', schema)).rejects.toThrow('Stagehand extract() failed: Schema validation failed');
    });
});

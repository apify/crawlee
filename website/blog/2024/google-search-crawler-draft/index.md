...
Additional parameters might be very useful for your specific case. For example, if you want to get results predominantly in Spanish, you can use - `https://www.google.com/search?q={YOUR_QUERY}&lr=lang_es`

## 2. Implementing the Crawler

Implemented for [`Crawlee for Python`](https://crawlee.dev/python) v0.4.0
...
Note that internal support for [`crawl_depth`](https://crawlee.dev/python/api/class/Request#crawl_depth) will be available soon.

Now let's implement the router code with the necessary data extraction:
...
async def default_handler(context: BeautifulSoupCrawlingContext) -> None:
    ...
    current_depth = context.request.user_data.get("current_depth", 0)
    crawl_depth = context.request.user_data.get("crawl_depth", current_depth + 1)
